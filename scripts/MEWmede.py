        ...
        try:
            ...
            for tag in heading_tags:
                ...
                if text and len(text) > 0:
                    ...
                    search_words=[r'Karl Marx',r'[Karl Marx]',r'Friedrich Engels',r'[Friedrich Engels]',r'Karl Marx/Friedrich Engels',r'[Karl Marx/Friedrich Engels]']
...
class BookVolume:
    ...
    def recont(self,recontent):
        ...
        if self.volume_number<=16 and self.volume_number not in range(5,9):
...
'''
    ...
    def create_volume_index_with_toc(self, volume):
        ...
        if i==20:
            ...
            ish4=",h4"
        if i==23:
            volumetext+="""<h4>Kritik der politischen Ökonomie</h4>
<p class="ctr"><i>Gewidmet<br>
meinem unvergeßlichen Freunde<br>
dem kühnen, treuen, edlen Vorkämpfer des Proletariats</i><br>
<b>Wilhelm Wolff</b><br>
<i>Geb. zu Tarnau, 21. Juni 1809. Gest. im Exil zu Manchester 9. Mai 1864</i></p>""" 
            ish4=",h4"
        if i==26:
            volumetext=f"<h3>Theorien über den Mehrwert, Teil 1 bis 3</h3>\n<h4>（剩余价值理论）</h4>"
            ish4=",h4"
        if i==40:
...
<head>
<title>{selftitle} BAND {i} – {volume.volume_name}</title>'''
        index_content+=r'''
<META content="text/html; charset=utf-8" http-equiv="Content-Type"/>
<META name="viewport" content="width=device-width, initial-scale=1.0"/>
<link rel="stylesheet" type="text/css" href="../mewde.css"/>
...
'''
        ...
        index_content+=f'''<h2 style="color: #DC3545;">{selftitle}<br>BAND {i}</h2>
{volumetext}
<h2>Inhalt</h2>
<nav class="TOC">
<ol>'''
        ...
        for chapter in volume.chapters:
            ...
            if titleIVXmatch:
                ...
                if not chapter['headings']:
                    continue
            elif not volIII and not ((i in range(5,9) or i in range(17,27)) and chapter['headings']):
                headings_final.append({
                    'tag': 'title',
                    'text': chapter['title'],
                    'level':1,
            ...
            if chapter['headings'] and (i in range(1,27) or i in [40]):
                for heading in chapter['headings']:
                    text = heading['text']
                    if not (i in range(5,9) or i in range(17,27)):
                        if (text.lower().replace("ß","ss").replace("ä","a").replace("ü","u") in chapter["title"].lower()) and text!="I":
                            continue
                        if titleIVXmatch and (text==titleIVXmatch.group(1) or text.replace(" ","")==titleIVXmatch.group(1).replace(".","").replace(" ","")):
                            continue
                    level = heading['level']
                    headid = heading['id']
                    if i in range(17,23) or i in range(7,9):
                        text=re.sub(r"(Karl Marx|Friedrich Engels|Karl Marx/Friedrich Engels)[\s]+",r"\1 – ",text,flags=re.DOTALL|re.IGNORECASE)
                        text=re.sub(r"[\s]+\]",r"]",text,flags=re.DOTALL|re.IGNORECASE)
                        text=re.sub(r"^([IVX]+?)[\s]+",r"\1. ",text,flags=re.DOTALL|re.IGNORECASE)
                    headings_final.append({
                        'tag':heading['tag'],
                        'text':text,
                        'level':level,
...
def main():
    ...
    book_dir = r"D:\马恩列总装\MEW-O"  # 修改为你的书籍根目录
    #output_dir = "./MEW10"  # 输出目录名
    output_dirs = [r"./mlread/docs/MEW",r"./MARX-ZH-CN-node/MEW"] 
    #output_dirs = [r"./de/MEW",r"./MARX-ZH-CN.github.io1/MEW"] 
