        ...
        try:
            ...
            for i, tag in enumerate(heading_tags):
                ...
                if text and len(text) > 0:
                    ...
                    # 确定标题级别
                    if len(heading_tags)==1 or level==1:
                        anchor_id=None
                    else:
                        anchor_id=tag.get('id')
                        if not anchor_id and level!=1:
...
class BookVolume:
    ...
    def extract_volume_number(self, volume_name):
        """从卷名中提取数字"""
        match = re.search(r'[0]*(\d+)', volume_name)
    ...
    def collect_images_from_content(self, html_content, source_file):
        ...
        for img_src in images:
            if not img_src.startswith(('http://', 'https://', 'data:')):
                ...
                if img_path.exists():
                    img_id = f"VL{self.volume_number:02d}-img{len(self.images) + 1:03d}"
    ...
    def scan_chapters(self):
        ...
        for link in links:
            ...
            try:
                ...
                content=re.sub(r'\s+(</h[\d]>)',r'\1'+'\n', content, flags=re.DOTALL | re.IGNORECASE)
                content=re.sub(r' <sup>',r'<sup>', content, flags=re.DOTALL | re.IGNORECASE)
...
"""
    ...
    def scan_book_structure(self, book_dir):
        ...
        for item in book_path.iterdir():
            if item.is_dir() and re.match(r'[0]*[\d]+', item.name):
...
<META charset="utf-8"/>
<META name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>Ленин ПСС издание 5-ТОМ {i:02d}-{volume.volume_name}</title>'''
        index_content+=r'''<link rel="stylesheet" type="text/css" href="/vil.css"/>
<script src="/mlr.js"></script>
<style type="text/css">
<!--
h1 {color: #DC3545;}
...
<body>'''
        index_content+=f'''<div style="COLOR:#DC3545;TEXT-ALIGN:CENTER;"><i>Пролетарии всех стран, соединяйтесь!</i></div>
        <h1>В. И. ЛЕНИН</h1>
        <h2>ТОМ {i}<br>{volume.volume_name}<br>СОДЕРЖАНИЕ</h2>
<nav class="TOC">
<ol>'''
        # 添加章节和标题的目录
        headings=[]
        ...
        def sub_navpoint(headings):
            ...
            while i<len(headings):
                ...
                if heading["text"] in [r"ПРИМЕЧАНИЯ",r"Указатель имен",r"Указатель литературных работ и источников, цитируемых и упоминаемых В. И. Лениным",r"Список изданий и документов, в редактировании которых принимал участие В. И. Ленин",r"Список неразысканных работ В. И. Ленина"] or "Даты жизни и деятельности В. И. Ленин" in heading['text'] or "Список работ В. И. Ленина, до настоящего времени не разысканных" in heading["text"] or "СПИСОК РАБОТ" in heading["text"]:
        ...
        for chapter in volume.chapters:
            ...
            if chapter['headings']:
                ...
                level=1
                if chapter['title']==r"Предисловие":
                    level=2
                headings_final.append({
                    'tag': 'title',
                    'text': chapter['title'],
    ...
    def build_epub_folder(self, output_folder):
        ...
        for volume in self.volumes:
            ...
            for outputpath in outputpaths:
                vol_output_Path=outputpath/ Path(f'{volume.volume_number}')
                vol_output_Path.mkdir(exist_ok=True)
                vol_paths.append(vol_output_Path)
            if volume.has_index:
                ...
                for vol_output_Path in vol_paths:
            ...
            for chapter in volume.chapters:
                ...
                if volume.volume_number in range(46,50):
                    fixed_content=re.sub(r'<a href="#[\s\S]+?(-[ab]*z[\S]+)" id="[\s\S]+?(-[ab]*z[\S]+)">',f'<a href="#{chapter['number']:03d}\\1" id="{chapter['number']:03d}\\2">',
                                         fixed_content ,flags=re.DOTALL | re.IGNORECASE)
                fixed_content=fixed_content.replace(r' id="onlyfortitle"',"")
                # 添加CSS链接
        ...
        for img in self.all_images:
            try:
                for output_path in outputpaths:
...
def main():
    ...
    # 配置参数
    book_title = "ЛЕНИН ПОЛНОЕ СОБРАНИЕ СОЧИНЕНИЙ"  # 修改为你的书名
    book_author = "Karl Marx & Friedrich Engels & 中共中央马克思、恩格斯、列宁、斯大林著作编译局"      # 修改为作者名
    book_dir = r"D:\马恩列总装\vilpss-ch"  # 修改为你的书籍根目录
    output_dir = [r"./LENINPSS-HTML-FB2-SPLITED/ru/VIL-FB2",r"./MARX-ZH-CN.github.io1/ru/VIL-FB2",r"./MARX-ZH-CN-node/ru/VIL-FB2"]   # 输出目录名
