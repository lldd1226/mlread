...
- 3. 使用 usepic.py 或 useaipdf2pa.py 打开对应卷的 pdf 页面，在 MEWbrief1.py 中，记录无法被程序机械拼接的跨页段落所在的页码：如果跨页段落的右下边缘处有句号、感叹号、问号、括号、脚注等无法用正则表达式处理的文本，或者说，只要跨页段落的右下边缘处不是逗号、连字符、单词，就该处的后一页页码记录在此卷的 mergepa 数组中，以供之后的合并脚本标记、拼接。
